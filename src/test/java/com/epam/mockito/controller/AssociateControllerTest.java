...
public class AssociateControllerTest {
    ...
    void validCreateAssociate() throws Exception {
        ...
        when(associateService.createAssociate(any())).thenReturn(associateResponse);
        mockMvc.perform(post("/rd/associates")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(new ObjectMapper().writeValueAsString(associateRequest)))

                .andExpect(status().isCreated());
        verify(associateService).createAssociate(any());
    }
    ...
    public void testGetAllAssociates() throws Exception {
        ...
        when(associateService.getAssociatesByGender(pageNumber - 1, pageSize, gender)).thenReturn(associateResponsePage);
    ...
    void getAssociateById() throws Exception {
        ...
        when(associateService.getAssociateById(1L)).thenReturn(associateResponse);
        mockMvc.perform(get("/rd/associates/get-associate/1"))
                .andExpect(status().isOk());
        verify(associateService).getAssociateById(1L);
    }
    ...
    void validUpdateAssociate() throws Exception {
        ...
        AssociateRequest associateRequest = PrepareTestObjects.getAssociateRequest();
        long associateId = 1L;
        when(associateService.updateAssociate(associateRequest,associateId)).thenReturn(associateResponse);

        mockMvc.perform(put("/rd/associates/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(new ObjectMapper().writeValueAsString(associateRequest)))
                .andExpect(status().isOk());
        verify(associateService).updateAssociate(associateRequest,associateId);
    }
    ...
    void associateExceptionTest() throws Exception {
        ...
        when(associateService.updateAssociate(associateRequest,associateId)).thenThrow(AssociateException.class);
    ...
    void testDeleteBatch() throws Exception{
        ...
        verify(associateService).deleteAssociate(any(Long.class));
    }
